...
from Utils_L2R.h_l2rMeasures import modelEvaluation, getQueries
...
def executeKeras (train, test, nneurons=100, nlayers=1, lr = 0.001, dropout = 0.2):
    #nneurons = 100
    #nlayers = 1
    #lr = 0.001
    #dropout = 0.2
    ...
    #    mlp = MLPRegressor(hidden_layer_sizes=sizes, learning_rate_init=lr, alpha=alpha)
    #    mlp.fit(X_, y_)

    model = keras.models.Sequential()

    # input layer
    model.add(keras.layers.normalization.BatchNormalization(input_shape=tuple([train.x.shape[1]])))
    ...
    model.fit(train.x, train.y, epochs=50)

    #file = open(sys.argv[2])
    #(X, y, feats) = load_rank_data(file)
    #file.close()
    #(X_test, index2feat) = to_densem(test, test_feats)
    #y_ = array(y)
    test.x = scaler.transform(test.x)
    resScore = model.predict(test.x)
...
def executeSckitLearn (l2r, train, test, seed, nTrees):
    ...
    if (l2r=="1" or l2r == "rf"):
        ...
        print (nTrees)
        clf = RandomForestRegressor(n_estimators=int(nTrees), max_features=0.3, max_leaf_nodes=100, min_samples_leaf=1, random_state=seed, n_jobs=-1)
        ...
        clf = GradientBoostingRegressor(n_estimators=nTrees[0], learning_rate=nTrees[1], max_depth=2, random_state=seed,  loss='ls')
...
def getL2RPrediction (l2r, fold, train, test, trainFile, testFile, parameters, mask, totalFeatures):
    ...
    if mask.count("1") == totalFeatures:
        ...
        if l2r == 6 or l2r == "lm":
            ...
            scores_test = executeExternalLib(l2r, trainFile, testFile, fold, "NDCG", parameters)
        elif l2r == 10 or l2r == "deep":
            # in parameters, the first one is nneurons, the secound is nlayers and the thierd learning rate
            scores_test = executeKeras(train, test, nneurons=parameters[0], nlayers=parameters[1], lr=parameters[2])
        else:
    ...
    if l2r == "6" or l2r =="lm":
        try:
            with open(out, 'w') as f:
                call(['/home/daniel/programs/quickrank/bin/quicklearn', '--algo', "LAMBDAMART", "--train", fTrain, "--train-metric",
                      "NDCG", "--train-cutoff", "10", "--test", fTest, "--test-metric", METRIC.upper(), "--test-cutoff", "10",
                      "--scores", fScore, "--num-trees", str(parameters[0]), "--shrinkage", "0.075", "--num-leaves", str(parameters[1])], stdout=f)
        except:
            result = [0]*10
            return result;
        ...
        try:
            ...
            with open(out, 'w') as f:
                call(["java", "-jar", "-Xms5002M", "-Xmx5024M", "/home/daniel/programs/RankLib-v2.1/bin/RankLib.jar",
                      "-round", str(parameters[0]), "-tolerance", str(parameters[1]), "-train", fTrain, "-test", fTest, "-ranker", l2r, "-save", fModel],
                    stdout=f)
            command.append(["java", "-jar", "-Xms5002M", "-Xmx5024M", "/home/daniel/programs/RankLib-v2.1/bin/RankLib.jar",
                      "-round", str(parameters[0]), "-tolerance", str(parameters[1]), "-train", fTrain, "-test", fTest, "-ranker", l2r, "-save", fModel])
